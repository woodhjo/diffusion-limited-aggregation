...
The probability that a particle sticks depends on how many neighbouring
cells are occupied where p = 0.125*N where N is the number of neighbouring
occupied cells.
...
class RealStickyDLA:
    def __init__(self, agg_radius, nbhood):
        ...
        for s in self.sites:
            self.sticky_lattice[self.seed[0] + s[0],
                                self.seed[1] + s[1]] = 0.125
    ...
    def _will_stick(self, point):
        return (np.random.random() < self.sticky_lattice[point[0], point[1]])
    ...
    def _update_aggregate(self, point, val):
        ...
        for s in self.sites:
            ...
            if self.lattice[row, col] == 0:
                self.sticky_lattice[row, col] += 0.125
...
# Make aggregate
model = RealStickyDLA(agg_radius, nbhood)
